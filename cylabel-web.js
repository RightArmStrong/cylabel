...
var attriText = function attriText(content, thisvalue) {
  ...
  if (results != null) {
    results.forEach((res) => {
      if (res != null) {
        ...
        var section = new attriTextSection(content, fontsize, fontcolor, fontweight, fontfamily, fontstyle );
  ...
  defaultSections.forEach((item)=>{
    if (item.length > 0) {
      ...
      var fontstyle = thisvalue ? thisvalue.defaultFontStyle :  undefined;
      var fontfamily = thisvalue ? thisvalue.defaultfontFamily :  undefined;
      var section = new attriTextSection(item, fontsize, fontcolor, fontweight, fontfamily,fontstyle );
      this.sections.push(section);
    }
  });
}
...
function animation(ctx, attri, percent) {  
  ...
  attri.forEach((item) => {
    if (item.atype == "static") {
      ctx.fillStyle=item.newchar.fontcolor;
      ctx.strokeStyle = item.newchar.fontcolor;
      ctx.font = item.newchar.fontweight + " " + item.newchar.fontsize + " " + item.newchar.fontfamily;
      ctx.fillText(item.newchar.content, item.new_x, item.new_y);
    }
    else {
      ctx.globalAlpha=1 - percent;
      ctx.fillStyle=item.oldchar.fontcolor;
      ctx.strokeStyle = item.oldchar.fontcolor;
      ctx.font = item.oldchar.fontweight + " " + item.oldchar.fontsize + " " + item.oldchar.fontfamily;
...
cylabel.prototype.drawWithoutAnimation = function(content, thisValue) {
...
cylabel.prototype.setText = function(content, hasAnimation) {
  ...
  for (var charIndex in attrChars) {
    if (attrChars[charIndex].atype == 'dynamic') {
      hasDynamic = true;
      break;
    }
  }
  if (hasDynamic) {
    ...
    this.drawWithoutAnimation(attrChars, this);
  }
